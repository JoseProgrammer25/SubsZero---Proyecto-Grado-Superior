...
$current_page_script = basename(__FILE__); // Esto será 'subscriptions.php'
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_action']) && $_POST['form_action'] == 'add_subscription') {
    ...
    if ($limit_reached) {
        ...
        $price = $_POST['price'];
        $billing_cycle = $_POST['billing_cycle']; // (ej. 'mensual', 'anual')
...
try {
    ...
    // Necesitas una referencia de variables para bind_param, ya que no puedes pasar un array directamente
    // Para entornos PHP > 5.6: call_user_func_array([$stmt_subs, 'bind_param'], array_merge([$sql_params_types_pag], $sql_params_values_pag));
    // Usando el operador de dispersión (spread operator) para entornos modernos:
...
<body class="bg-gray-50">
    <div class="flex min-h-screen">
        ...
        <aside id="sidebar" class="fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white shadow-lg flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
            ...
            <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
                ...
                <a href="subscriptions.php" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors <?php echo ($current_page_script == 'subscriptions.php') ? 'bg-blue-700 text-white' : 'text-gray-700 hover:bg-gray-50'; ?>">
                        ...
                        <?php if ($limit_reached): ?>
                            <div class="relative group mt-4 sm:mt-0">
                                <button class="bg-gray-400 text-white px-4 py-2.5 rounded-lg text-sm font-medium flex items-center cursor-not-allowed" disabled>
                                    ...
                                    Añadir Suscripción (<?php echo $user_total_subs; ?>/10)
                                </button>
                                <span class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-xs px-3 py-1.5 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-10 text-center">
                                    Límite de 10 suscripciones alcanzado para usuarios Estándar.
                                </span>
                            </div>
                        <?php else: ?>
                            <button type="button" id="open-modal-btn" class="bg-blue-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-800 transition-colors flex items-center mt-4 sm:mt-0">
                                ...
                                Añadir Suscripción
                                <?php if ($role == 'user'): ?>
                                    <span class="ml-2 text-xs font-semibold">(<?php echo $user_total_subs; ?>/10)</span>
                                <?php endif; ?>
                            </button>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Asegurándonos de que el overlay tiene el ID correcto
