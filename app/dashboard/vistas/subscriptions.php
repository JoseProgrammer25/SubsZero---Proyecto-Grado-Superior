...
// ... (Este bloque es idéntico, no lo repito para abreviar) ...
$stmt = $conn->prepare("
    SELECT 
        u.username, u.email,
...
$current_page = basename($_SERVER['PHP_SELF']); // Esto será 'subscriptions.php'

// --- ¡MODIFICADO! LÓGICA DE LÍMITE DE USUARIO (REQUERIMIENTO 3 y 4) ---
$limit_reached = false;
$user_total_subs = 0;
if ($role == 'user') {
    ...
    if ($user_total_subs >= 15) {
        $limit_reached = true;
    }
}

// --- ¡MODIFICADO! LÓGICA DE PAGINACIÓN Y FILTROS (REQUERIMIENTO 1 y 2) ---
$subs_per_page = 6; // 6 suscripciones por página
...
// Construir la consulta con filtros dinámicamente
...
if ($search_term !== '') {
    ...
    $sql_params_types .= "s";
    $sql_params_values[] = "%" . $search_term . "%"; // Búsqueda flexible
}
...
try {
    // 1. Obtener el CONTEO TOTAL FILTRADO (para calcular total de páginas)
    ...
    // 2. Obtener las suscripciones PAGINADAS y FILTRADAS
    $sql_params_types .= "ii"; // Añadir tipos para LIMIT y OFFSET
    $sql_params_values[] = $subs_per_page;
    $sql_params_values[] = $offset;

    $stmt_subs = $conn->prepare("
        SELECT 
            s.id, s.name, s.description, s.price, s.billing_cycle, s.next_billing_date,
    ...
    $stmt_subs->bind_param($sql_params_types, ...$sql_params_values);
    $stmt_subs->execute();
    $result_subs = $stmt_subs->get_result();
    $subscriptions = $result_subs->fetch_all(MYSQLI_ASSOC);
    $stmt_subs->close();

    // 3. Obtener todas las categorías para el filtro (sin cambios)
    ...
    <div class="flex min-h-screen">
        ...
        <aside id="sidebar" class="fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white shadow-lg flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
            ...
            <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
                <a href="../dashboard.php" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors <?php echo ($current_page == 'dashboard.php') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50'; ?>">
                    <svg class="mr-3 h-5 w-5 flex-shrink-0 <?php echo ($current_page == 'dashboard.php') ? 'text-gray-900' : 'text-gray-500'; ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                    </svg>
                    Dashboard
                </a>
                
                <a href="subscriptions.php" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors <?php echo ($current_page == 'subscriptions.php') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50'; ?>">
                ...
                <a href="statistics.php" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors <?php echo ($current_page == 'statistics.php') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50'; ?>">
                ...
                <a href="notifications.php" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors <?php echo ($current_page == 'notifications.php') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50'; ?>">
                ...
                <a href="forums.php" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors <?php echo ($current_page == 'forums.php') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50'; ?>">
                ...
                <a href="settings.php" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors <?php echo ($current_page == 'settings.php') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50'; ?>">
                        ...
                        <?php if ($limit_reached): ?>
                            <div class="relative group mt-4 sm:mt-0">
                                ...
                                <span class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-xs px-3 py-1.5 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-10 text-center">
                                    Límite de 15 suscripciones alcanzado para usuarios gratuitos.
                                </span>
                            </div>
                        <?php else: ?>
                            <a href="add_subscription.php" class="bg-blue-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-800 transition-colors flex items-center mt-4 sm:mt-0">
                                <svg class="mr-2 h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"/>
                                </svg>
                                Añadir Suscripción
                    ...
                    <div id="subscription-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <?php if (count($subscriptions) > 0): ?>
                            ...
                            <?php foreach ($subscriptions as $sub): ?>
                                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col subscription-card" data-name="<?php echo strtolower(htmlspecialchars($sub['name'])); ?>" data-category="<?php echo $sub['category_id']; ?>">
